...
def main():
        # 사용자 입력 받기
        keyword = st.text_input("시군구-번지 일부 입력 (예: 강원특별자치도 강릉시): ")
        
        forecast_model = RealEstateForecast(model_num=1)
        matching_city_lot_nums = forecast_model.search_estate_data(keyword)['시군구번지'].unique()

        if len(matching_city_lot_nums) == 0:
                st.write("일치하는 시군구-번지가 없습니다.")
                return
